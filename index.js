...
const io = socketIo(server, {
  cors: {
    origin: 'https://lightning-chat-frontend.vercel.app', // Allow the frontend URL
    methods: ['GET', 'POST'], // Allowed methods for socket connections
    allowedHeaders: ['Content-Type', 'Authorization'], // Allowed headers for the socket
...
app.get('/api/groups', async (req, res) => {
  try {
    const groups = await Group.find().populate("members", "name status");
...
io.on('connection', (socket) => {
  ...
  // Logout route (inside socket connection)
